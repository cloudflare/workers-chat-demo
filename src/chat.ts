...
async function handleErrors(request: Request, func: () => unknown) {
  try {
    return await func();
  } catch (err) {
...
async function handleApiRequest(path: string[], request: Request, env: Env) {
  ...
  switch (path[0]) {
    case "room": {
      ...
      const { cf: _cf, ...requestInit } = request;
  ...
  async handleSession(webSocket: WebSocket, ip: string) {
    ...
    webSocket.addEventListener("message", (msg) => {
      try {
        ...
        let data = JSON.parse(
          typeof msg.data === "string" ? msg.data : textDecoder.decode(msg.data)
        ) as Data;
...
export class RateLimiter {
  ...
  // next action.
  async fetch(request: Request) {
    return await handleErrors(request, () => {
